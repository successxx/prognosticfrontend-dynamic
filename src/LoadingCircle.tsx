...
const LoadingIndicator: React.FC = () => {
  ...
  // Progress bar + rotating messages effect
  useEffect(() => {
    // Clear old intervals if any
    if (progressIntervalRef.current) {
      window.clearInterval(progressIntervalRef.current);
    }
    // Progress bar increments based on messageIndex
  ...
  // Randomly pick snap-in classes (4 variants) for each module
  const snapClasses = [
    styles.snapIn1,
    styles.snapIn2,
    styles.snapIn3,
  ...
  function getRandomSnapClass(i: number) {
    // create some pseudo-random index for variety
    return snapClasses[i % snapClasses.length];
  }

  // We also randomize the delay classes from 0..11
  const delayClasses = [
